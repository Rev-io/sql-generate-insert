...
Modified by Jordan @ Rev.io to fix a few issues in the script - likely needs to be ported back to the repo & and put in as a PR.
...
BEGIN
  ...
  CREATE TABLE #Results (ID int IDENTITY(1, 1), TableRow nvarchar(max));
  ...
  IF EXISTS (SELECT * FROM @TableData)
  BEGIN
    IF @GenerateProjectInfo = 1
    BEGIN
      INSERT INTO #Results (TableRow)
  ...
  END ELSE BEGIN
    INSERT INTO #Results (TableRow)
    SELECT N'--No data found for ' + COALESCE(@TargetObjectName,@ObjectName);
  END
